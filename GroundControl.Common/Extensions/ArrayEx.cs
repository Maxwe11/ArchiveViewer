namespace GroundControl.Common.Extensions
{
    using System;

    public static class ArrayEx
    {
        #region CRC6

        private static readonly byte[] Crc16Hi =
        {
            
        };

        private static readonly byte[] Crc16Lo =
        {
            
        };

        public static ushort CalcCrc16(this byte[] data)
        {
            byte crcHi = 0xFF;
            byte crcLo = 0xFF;
            int index = 0, length = data.Length;

            while (length-- > 0)
            {
                var crc16 = (ushort)(crcHi ^ data[index++]);
                crcHi = (byte)(crcLo ^ Crc16Hi[crc16]);
                crcLo = Crc16Lo[crc16];
            }

            return (ushort)(crcHi << 8 | crcLo);
        }

        public static ushort CalcCrc16(this ushort[] data)
        {
            byte crcHi = 0xFF;
            byte crcLo = 0xFF;
            int index = 0, length = data.Length;

            while (length-- > 0)
            {
                var crc16 = (ushort)(crcHi ^ data[index].Low());
                crcHi = (byte)(crcLo ^ Crc16Hi[crc16]);
                crcLo = Crc16Lo[crc16];

                crc16 = (ushort)(crcHi ^ data[index++].Hi());
                crcHi = (byte)(crcLo ^ Crc16Hi[crc16]);
                crcLo = Crc16Lo[crc16];
            }

            return (ushort)(crcHi << 8 | crcLo);
        }

        #endregion

        #region CRC32

        public static uint CalcCrc32(this ushort[] data)
        {
            throw new NotImplementedException();
        }

        /*
        private uint CRC32(byte[] bytes)
        {
            const int WIDTH = 8 * sizeof(uint);
            const int TOPBIT = 1 << (WIDTH - 1);
            const int POLYNOMIAL = 0x04C11DB7;

            uint remainder = 0xFFFFFFFF;

            // Perform modulo-2 division, a byte at a time.
            for (int i = 0; i < bytes.Length; ++i)
            {
              // Bring the next byte into the remainder.
                remainder ^= (uint)(bytes[i] << (WIDTH - 8));

                // Perform modulo-2 division, a bit at a time.
                for (byte bit = 8; bit > 0; --bit)
                {
                     //Try to divide the current data bit.
                    if ((remainder & TOPBIT) != 0)
                    {
                        remainder = (remainder << 1) ^ POLYNOMIAL;
                    }
                    else
                    {
                        remainder = (remainder << 1);
                    }
                }
            }

         //The final remainder is the CRC result.
         
            return (remainder ^ 0xFFFFFFFF);
        }
        */

        #endregion

        public static int GetInt32(this byte[] data, int index = 0)
        {
            int result = data[index++];
            
            result |= data[index++] << 8;
            result |= data[index++] << 16;
            result |= data[index] << 24;
            
            return result;
        }

        public static uint GetUInt32(this byte[] data, int index = 0)
        {
            uint result = data[index++];

            result |= (uint)(data[index++] << 8);
            result |= (uint)(data[index++] << 16);
            result |= (uint)(data[index] << 24);

            return result;
        }

        public static int GetInt32(this ushort[] data, int index = 0)
        {
            int result = data[index++];
            return (result | (data[index] << 16));
        }

        public static uint GetUInt32(this ushort[] data, int index = 0)
        {
            uint result = data[index++];
            return (result | (uint)(data[index] << 16));
        }
    }
}
